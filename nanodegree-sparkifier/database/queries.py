...
def createDataQualityQueries(listOfTableNames):
    """
    Create some queries to check that data was correctly read.
    Return a list of check-queries and add an expected result

    Args:
        listOfTableNames (list): a list of tablenames as generated by getTableNames

    Returns:
        list: a list of tuples. Each tuple contains the query and then the expected (single) result
    """
    ...
    weatherTuple = (weatherNumberQuery, 100)
    listToReturn.append(weatherTuple)

    # we expect each 10 age groups in the mortality data
    mortalityQuery = """
    SELECT COUNT(*) FROM (
    ...
    mortTuple = (mortalityQuery, 10)
    listToReturn.append(mortTuple)
